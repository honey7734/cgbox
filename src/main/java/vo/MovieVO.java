package vo;

public class MovieVO {
	private String movie_no		 ;
	private String movie_name    ;
	private String movie_open    ;
	private String movie_summary ;
	private int    movie_grade   ;
	private String movie_running ;
	private int    movie_audience;
	private int    movie_trailer ;
	private String movie_link    ;
	private String movie_img     ;
	
	
	public String getMovie_no() {
		return movie_no;
	}
	public void setMovie_no(String movie_no) {
		this.movie_no = movie_no;
	}
	public String getMovie_name() {
		return movie_name;
	}
	public void setMovie_name(String movie_name) {
		this.movie_name = movie_name;
	}
	public String getMovie_open() {
		return movie_open;
	}
	public void setMovie_open(String movie_open) {
		this.movie_open = movie_open;
	}
	public String getMovie_summary() {
		return movie_summary;
	}
	public void setMovie_summary(String movie_summary) {
		this.movie_summary = movie_summary;
	}
	public int getMovie_grade() {
		return movie_grade;
	}
	public void setMovie_grade(int movie_grade) {
		this.movie_grade = movie_grade;
	}
	public String getMovie_running() {
		return movie_running;
	}
	public void setMovie_running(String movie_running) {
		this.movie_running = movie_running;
	}
	public int getMovie_audience() {
		return movie_audience;
	}
	public void setMovie_audience(int movie_audience) {
		this.movie_audience = movie_audience;
	}
	public int getMovie_trailer() {
		return movie_trailer;
	}
	public void setMovie_trailer(int movie_trailer) {
		this.movie_trailer = movie_trailer;
	}
	public String getMovie_link() {
		return movie_link;
	}
	public void setMovie_link(String movie_link) {
		this.movie_link = movie_link;
	}
	public String getMovie_img() {
		return movie_img;
	}
	public void setMovie_img(String movie_img) {
		this.movie_img = movie_img;
	}
	
	
	@Override
	public String toString() {
		return "MovieVO [movie_no=" + movie_no + ", movie_name=" + movie_name + ", movie_open=" + movie_open
				+ ", movie_summary=" + movie_summary + ", movie_grade=" + movie_grade + ", movie_running="
				+ movie_running + ", movie_audience=" + movie_audience + ", movie_trailer=" + movie_trailer
				+ ", movie_link=" + movie_link + ", movie_img=" + movie_img + "]";
	}
	
}
